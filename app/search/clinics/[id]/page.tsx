...
export default function ClinicDetailPage() {
  ...
  const [entity, setEntity] = useState<ClinicDetail | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch(`/api/search/clinics?limit=100`)
      .then(r => r.json())
      .then(j => {
        if (j.success) {
          const found = j.data?.find((e: ClinicDetail) => e.id === id)
  ...
  if (!entity) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4">
        ...
        <h2 className="font-bold text-[#001E40] text-xl">Clinic not found</h2>
        <Link href="/search/clinics" className="text-[#0C6780] text-sm hover:underline">← Back to clinics</Link>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-6">
          <Link href="/search/clinics" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#0C6780] mb-4">
            <FaArrowLeft size={11} /> Back to Clinics
          </Link>

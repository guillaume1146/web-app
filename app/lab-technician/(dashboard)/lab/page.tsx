...
import { FaFlask, FaClipboardList, FaFileAlt, FaBriefcaseMedical } from 'react-icons/fa'

const TestsContent = dynamic(() => import('../tests/page'), { ssr: false, loading: () => <TabLoading /> })
const BookingRequestsContent = dynamic(() => import('../booking-requests/page'), { ssr: false, loading: () => <TabLoading /> })
const ResultsContent = dynamic(() => import('../results/page'), { ssr: false, loading: () => <TabLoading /> })
const CatalogContent = dynamic(() => import('../services/page'), { ssr: false, loading: () => <TabLoading /> })
...
const TABS = [
 ...
 { id: 'catalog', label: 'Catalog', icon: FaBriefcaseMedical },
] as const

type TabId = typeof TABS[number]['id']

export default function LabTechLabPage() {
 ...
 >
 <Icon className={isActive ? 'text-cyan-600' : 'text-gray-400'} />
 {tab.label}
 </button>
 )
 })}
 ...
 {isActive && <div className="w-1 h-1 bg-cyan-600 rounded-full mt-0.5" />}
 </button>
 )
 })}
 </div>

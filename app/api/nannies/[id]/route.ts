...
/**
 * GET /api/nannies/[id]
 * Public nanny profile (for patients viewing nanny details).
 */
  ...
  try {
    ...
    const nannyProfile = await prisma.nannyProfile.findUnique({
      ...
      select: {
        ...
        nannyServiceCatalog: {
          where: { isActive: true },
          select: { id: true, serviceName: true, description: true, price: true, category: true, ageRange: true },
        },
      },
    })
    if (!nannyProfile) {

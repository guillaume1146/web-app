...
export class StockCheckStrategy implements StepFlagHandler {
  ...
  async validate(ctx: TransitionContext): Promise<{ valid: boolean; errors: string[] }> {
    ...
    for (const item of items) {
      const inv = await this.prisma.providerInventoryItem.findFirst({
        where: { id: item.itemId, providerUserId: ctx.providerUserId },
        select: { name: true, quantity: true, inStock: true, isActive: true },
      });
      if (!inv || !inv.isActive) unavailable.push(`Item not found: ${item.itemId}`);
      else if (!inv.inStock || inv.quantity < item.quantity) unavailable.push(`${inv.name}: only ${inv.quantity} available (requested ${item.quantity})`);
    }
    return { valid: unavailable.length === 0, errors: unavailable };
  }
}

...
export default function ProvidersSection() {
  ...
  useEffect(() => {
    fetch('/api/roles?searchEnabled=true')
      ...
      .then(async (json) => {
        ...
        const roleList: RoleData[] = json.data.filter((r: RoleData) =>
          r.providerCount > 0 || process.env.NODE_ENV === 'development'
        )
        setRoles(roleList)

        // Load providers for each role in parallel, max 8 per role
        const grouped = await Promise.all(
          roleList.map(async (role) => {
            try {
              const res = await fetch(`/api/search/providers?type=${role.code}&limit=8`)
              const data = await res.json()
              const providers: ProviderCard[] = (data.providers || data.data || []).map((p: any) => ({
                id: p.id,
  ...
  const filteredGroups = useMemo(() => {
    let list = groups
    if (activeRole !== 'ALL') list = list.filter(g => g.code === activeRole)
    if (searchQuery.trim()) {
        ...
        {loading ? (
          ...
          filteredGroups.map(group => (
            <HorizontalScrollRow
              key={group.code}
              title={group.label}
              subtitle={`${group.providers.length} provider${group.providers.length !== 1 ? 's' : ''} available`}
              icon={<span className="text-xl">{PROVIDER_EMOJI[group.code] ?? '👨‍⚕️'}</span>}
              seeAllHref={`/search/${group.slug}`}
              seeAllLabel="See All"
            >
              {group.providers.map(provider => (
                <ProviderCard key={provider.id} provider={provider} color={group.color} slug={group.slug} />
              ))}
            </HorizontalScrollRow>
          ))
        )}
...
function ProviderCard({ provider, color, slug }: { provider: ProviderCard; color: string; slug: string }) {
  const bgSoft = hex2rgba(color, 0.08)

  return (
    <Link
      href={`/profile/${provider.id}`}
      className="group flex-shrink-0 snap-start w-[140px] sm:w-44 flex flex-col items-center

...
interface WorkflowBuilderProps {
  ...
  /** Pre-populate the builder with data generated by the wizard. Only
   *  applied once on mount (ref-guarded). */
  wizardData?: {
    name: string
    description: string
    serviceMode: string
  ...
  /** Called when the user clicks "Configure with Wizard". If not provided,
   *  the wizard button is hidden. */
...
export default function WorkflowBuilder({ backHref, initialData, lockedProviderType, showAdminFields, onSave, wizardData, onRequestWizard }: WorkflowBuilderProps) {
  ...
  /** Guard so wizard pre-population only fires once, even in StrictMode. */
  const wizardApplied = useRef(false)

  // Pre-populate builder state from wizard-generated data (fires at most once).
  useEffect(() => {

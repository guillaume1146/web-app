...
interface ProviderSearchPageConfig {
  /** UserType enum value, e.g. 'CAREGIVER' */
  providerType: string
  /** Page title, e.g. 'Find Caregivers' */
  title: string
  /** Singular label, e.g. 'Caregiver' */
  singularLabel: string
  /** URL slug for search detail pages, e.g. 'caregivers' */
  slug: string
  /** Border color class for avatar */
  accentColor?: string
  /** Search placeholder examples */
  searchExamples?: string[]
}

const ProviderCard = ({ provider, slug, accentColor }: { provider: Provider; slug: string; accentColor: string }) => {
  return (
      ...
      <div className="p-4 sm:p-5 flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex flex-col items-stretch sm:items-end gap-2 flex-shrink-0 sm:border-l sm:border-gray-100 sm:pl-4 border-t sm:border-t-0 border-gray-100 pt-3 sm:pt-0">
          <div className="flex items-center gap-2">
            ...
            <AuthBookingLink type="doctor" providerId={provider.id} className="flex-1 sm:flex-none bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors text-center">
              Book
            </AuthBookingLink>
...
function ProviderSearchContent({ config }: { config: ProviderSearchPageConfig }) {
  ...
  const examples = config.searchExamples || [
    `Find ${config.singularLabel.toLowerCase()} near me`,
    `${config.singularLabel} for consultation`,
    `Experienced ${config.singularLabel.toLowerCase()}`,
  ]
            ...
            <form onSubmit={handleSearch} className="flex flex-col gap-4">
              <div className="relative">
                <input
                  ...
                  placeholder={`Search for ${config.singularLabel.toLowerCase()}s by name, specialty, or location...`}
              ...
              <button
                ...
                type="submit"
                disabled={isLoading}
                className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-8 py-2.5 rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-200 font-medium flex items-center justify-center gap-2 shadow-lg disabled:opacity-50"
              >
                {isLoading ? (
                  <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> Searching...</>
                ) : (
                  <><FaSearch /> {config.title}</>
                )}
              </button>
            </form>
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {examples.map((example, i) => (
                    <button key={i} onClick={() => setSearchQuery(example)}

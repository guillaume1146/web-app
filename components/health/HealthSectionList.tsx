...
interface HealthSectionListProps {
  title: string
  icon: IconType
  apiUrl: string // e.g. "/api/patients/{userId}/appointments"
...
export default function HealthSectionList({ title, icon: Icon, apiUrl, mapData, emptyMessage }: HealthSectionListProps) {
  ...
  return (
    <div className="p-4">
      ...
      {items.length === 0 ? (
        <p className="text-center py-8 text-gray-400 text-sm">{emptyMessage || `No ${title.toLowerCase()} yet.`}</p>
      ) : (
